...
# 테크 블로그 홈 url 요청
base_res = requests.get("https://medium.com/coupang-engineering/kr/home", user_agent)
base_soup = BeautifulSoup(base_res.text, "html.parser")

for menu in base_soup.find("ul", "js-collectionNavItems").find_all("li")[:5]:
    # 메뉴 별 url
    ...
    for post in page_soup.find_all("div","streamItem streamItem--postPreview js-streamItem"):
        # 각 post별 url
...
# 수집된 데이터 파일 저장 
with open("../data/coupang.pkl", "wb") as f:
    pickle.dump(data, f)